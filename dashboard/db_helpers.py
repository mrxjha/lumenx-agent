...
def approve_draft(
    ...
    send_to_lumenx: bool = True,
) -> tuple[bool, Optional[str]]:
    """Approve a draft. Updates local DB, optionally POSTs the reply to LumenX.

    Returns (sent_remote, error_message). `sent_remote` is False if we skipped
    or failed to call LumenX; `error_message` describes the failure.
    """
    conn = get_connection()
    try:
    ...
    if not send_to_lumenx or not thread_id:
        return False, None
