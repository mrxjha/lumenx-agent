"""LumenX Auto-Reply Agent — human-review + cost dashboard.
...
st.set_page_config(
    page_title="LumenX Agent — Review & Cost",
    page_icon="✨",
    layout="wide",
)
...
def _render_one_draft(d: dict) -> None:
    ...
    with col_l:
        ...
        send_remote = c3.checkbox("POST to LumenX", value=True, key=f"send_remote_{d['id']}")
        if c1.button("✓ Approve & send", key=f"approve_{d['id']}", type="primary"):
            sent, err = approve_draft(d["id"], edited, thumbs_value, correction, send_to_lumenx=send_remote)
            if err:
                st.error(f"Saved locally, but LumenX POST failed: {err}")
            elif sent:
                st.success(f"Draft #{d['id']} approved and sent to LumenX.")
            else:
